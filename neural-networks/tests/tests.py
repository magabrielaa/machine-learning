...
class TestNetwork(unittest.TestCase):
    ...
    def TestBackPropDropoutUpdated(self):
        # =====================================================
        # BackProp test without regularization
        # =====================================================
        np.random.seed(1234)
        ...
        self.assertAlmostEqual(nn_noreg.weights[0][0, 0], 3.44111488)
        self.assertAlmostEqual(nn_noreg.weights[0][0, 1], 2.93712279)
        self.assertAlmostEqual(nn_noreg.weights[0][1, 0], -5.11225363)
        self.assertAlmostEqual(nn_noreg.weights[0][1, 1], 1.14828173)
        self.assertAlmostEqual(nn_noreg.weights[0][2, 0], -3.90980924)
        self.assertAlmostEqual(nn_noreg.weights[0][2, 1], 2.40544399)

        self.assertAlmostEqual(nn_noreg.biases[0][0][0], -2.8390044)
        self.assertAlmostEqual(nn_noreg.biases[0][1][0], -3.37534026)
        self.assertAlmostEqual(nn_noreg.biases[0][2][0], -2.93458673)

        self.assertAlmostEqual(nn_noreg.weights[1][0, 0], -1.7303108)
        self.assertAlmostEqual(nn_noreg.weights[1][1, 0], 1.73069058)
        self.assertAlmostEqual(nn_noreg.weights[1][0, 1], -1.85042449)
        self.assertAlmostEqual(nn_noreg.weights[1][1, 1], 1.85168616)
        self.assertAlmostEqual(nn_noreg.weights[1][0, 2], -1.48760171)
        self.assertAlmostEqual(nn_noreg.weights[1][1, 2], 1.48760508)

        self.assertAlmostEqual(nn_noreg.biases[1][0][0], 0.71095957)
        self.assertAlmostEqual(nn_noreg.biases[1][1][0], -0.71112266)
...
def run_test_suite(name, ctor):
    if name == "prob 2.3":
        ...
        assert (
            unittest.TextTestRunner(verbosity=2).run(prob3).wasSuccessful()
        ), "one or more tests for prob 2.3 failed"
    elif name == "prob 2.4":
        prob3 = unittest.TestSuite()
        ...
        assert (
            unittest.TextTestRunner(verbosity=2).run(prob3).wasSuccessful()
        ), "one or more tests for prob 2.4 failed"
    elif name == "prob 3":
        prob3 = unittest.TestSuite()
        for test in [
            "TestBackPropWithoutRegularization",
            "TestBackPropWithRegularization",
        ]:
            prob3.addTest(TestNetwork(test, ctor))
        assert (
            unittest.TextTestRunner(verbosity=2).run(prob3).wasSuccessful()
        ), "one or more tests for prob 3 failed"
    elif name == "prob 4":
        prob4 = unittest.TestSuite()
        for test in [
            "TestBackPropDropoutWithoutRegularization",
            "TestBackPropDropoutUpdated",
        ]:
            prob4.addTest(TestNetwork(test, ctor))
        res = unittest.TextTestRunner(verbosity=2).run(prob4)
        assert len(res.failures) <= 1, "both tests for prob 4 failed"
    elif name == "new prob 4":
        ...
        assert (
            unittest.TextTestRunner(verbosity=2).run(prob5).wasSuccessful()
        ), "one or more tests for prob 5 failed"
    else:
